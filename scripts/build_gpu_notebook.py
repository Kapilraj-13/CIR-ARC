...
cells = [
    {
        ...
        "metadata": {},
        "source": [
            "# CIR-ARC-3B: Dual NVIDIA Tesla T4 GPU Training Harness (3.000B Parameters)\n",
            "### Kaggle Dual T4 GPU (T4 x 2, 30 GB VRAM total) Production Run\n",
            "\n",
            "- **Parameters**: Exactly 3,000,000,000 across 10 cognitive faculties.\n",
            "- **Hardware**: Dual NVIDIA Tesla T4 GPUs (15 GB + 15 GB = 30 GB VRAM).\n",
            "- **Precision**: FP16 Mixed Precision on Turing Tensor Cores via `torch.amp.autocast`.\n",
            "- **Parallelism**: Multi-GPU `torch.nn.DataParallel` distributing batches across both GPUs.\n",
    ...
    },
    {
        ...
        "outputs": [],
        "source": [
            "# Cell 2: Connect to Dual T4 GPU Hardware Accelerators\n",
            "import torch\n",
            "\n",
    ...
    },
    {
        ...
        "outputs": [],
        "source": [
            "# Cell 5: Dual T4 GPU Training Loop (T4 x 2 with DataParallel + FP16 + Adafactor)\n",
            "import time, math, os\n",
            "import torch\n",
            ...
            "from cir_arc.neural.models.cir_arc_3b import CirArc3B, CirArc3BConfig\n",
            "\n",
            "# 1. Instantiate 3.000B Parameter Model in FP16\n",
            ...
            "    num_gpus = torch.cuda.device_count()\n",
            "    if num_gpus > 1:\n",
            "        print(f'[MULTI-GPU] Distributing batches across {num_gpus} GPUs via DataParallel...')\n",
            "        model = nn.DataParallel(base_model, device_ids=list(range(num_gpus)))\n",
            "    else:\n",
            "        model = base_model\n",
            "else:\n",
            "    model = base_model\n",
            "print('      [SUCCESS] 3.000B model loaded into GPU VRAM!', flush=True)\n",
            "\n",
            "# 2. High-Throughput Parallel DataLoader\n",
            "# Total batch size = 4 (2 per T4 GPU)\n",
            "batch_size = 4 if torch.cuda.device_count() >= 2 else 2\n",
            ...
            "optimizer = Adafactor(base_model.parameters(), lr=2e-4, weight_decay=0.1)\n",
            ...
            "\n",
            "    entrapment_risk = model_out['entrapment_risk'].squeeze(-1)\n",
            "    target_risk = batch['is_negative'].to(entrapment_risk.device)\n",
            "    l_safety = F.binary_cross_entropy(entrapment_risk, target_risk)\n",
            "\n",
            "    pred_state = model_out['world_model']['predicted_state']\n",
            "    cog_state = model_out['cognitive_state'].detach()\n",
            ...
            "\n",
            "print('\\n' + '=' * 65, flush=True)\n",
            "print(f'[3/4] STARTING CIR-ARC-3B DUAL T4 GPU TRAINING ({num_epochs} Epochs)', flush=True)\n",
            "print('=' * 65, flush=True)\n",
            "\n",
