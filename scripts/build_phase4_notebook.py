        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# Ensure official ARC-AGI benchmark repository is cloned for real task evaluation & fine-tuning\n",
                    "official_arc_dir = os.path.join(repo_path, 'data', 'official', 'ARC-AGI')\n",
                    "if not os.path.exists(official_arc_dir) or len(os.listdir(official_arc_dir)) == 0:\n",
                    "    print('Cloning official ARC-AGI benchmark dataset from Francois Chollet...')\n",
                    "    os.makedirs(os.path.dirname(official_arc_dir), exist_ok=True)\n",
                    "    !git clone --depth 1 https://github.com/fchollet/ARC-AGI.git data/official/ARC-AGI\n",
                    "\n",
                    "print(f'Working directory: {os.getcwd()}')\n",
                    "print(f'CIR-ARC package located: {os.path.exists(os.path.join(repo_path, \"src\", \"cir_arc\"))}')\n",
                    "print(f'Official ARC-AGI located: {os.path.exists(official_arc_dir)}')"
                ]
            },
            {
                "cell_type": "markdown",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# Ensure scaled synthetic dataset (40,000+ tasks) is generated on disk\n",
                    "train_files = list(Path(train_data_dir).rglob('*.json')) if os.path.exists(train_data_dir) else []\n",
                    "if len(train_files) < 20000:\n",
                    "    print('=' * 75)\n",
                    "    print('Generating scaled procedural ARC reasoning dataset (40,000+ tasks)...')\n",
                    "    print('=' * 75)\n",
                    "    !python scripts/generate_data.py --n_per_rule 2500 --n_per_pair 1250\n",
                    "\n",
                    ...
                    ")\n",
                    "\n",
                    "# 1. Procedural synthetic dataset with D8 + color permutation augmentations\n",
                    "synth_train = ReasoningArcDataset(data_dir=train_data_dir, max_samples=None, seed=42, augment=True)\n",
                    "print(f'Loaded {len(synth_train):,} procedural synthetic tasks (with D8 augmentation)')\n",
                    "\n",
                    "# 2. Official ARC-AGI benchmark dataset\n",
                    "official_path = Path('data/official/ARC-AGI/data')\n",
                    "if official_path.exists():\n",
                    "    official_train = OfficialArcDataset(task_dir=official_path, split='training', augment=True, seed=42)\n",
                    "    print(f'Loaded {len(official_train):,} official ARC-AGI benchmark training tasks (with D8 augmentation)')\n",
                    "    train_dataset = ConcatDataset([synth_train, official_train])\n",
                    "else:\n",
                    "    train_dataset = synth_train\n",
                    "print(f'Total unified training instances: {len(train_dataset):,}')\n",
                    "\n",
                    "# 3. Held-out validation dataset\n",
                    "val_dataset = ReasoningArcDataset(data_dir=val_data_dir, max_samples=None, seed=101, augment=False) if os.path.exists(val_data_dir) else None\n",
                    "if val_dataset:\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "print('Optimizer, CosineAnnealingLR Scheduler, and GradScaler initialized successfully.')"
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# Scaled multi-epoch curriculum training on Kaggle GPUs (50-100 Epochs)\n",
                    ...
                    "\n",
                    "num_epochs = 50  # Set to 50 or 100 for deep convergence\n",
                    "max_batches = min(len(train_loader), 200)\n",
                    "best_accuracy = 0.0\n",
                    "best_loss = float('inf')\n",
                    "print(f'Starting Scaled Multi-Epoch Training ({num_epochs} Epochs on Dual GPUs, {max_batches} batches/epoch)...')\n",
                    "for epoch in range(1, num_epochs + 1):\n",
                    "    current_lr = scheduler.get_last_lr()[0]\n",
                    "    print(f'\\n>>> Running Epoch {epoch}/{num_epochs} [LR: {current_lr:.2e}]')\n",
                    ...
                    "    tot_loss = train_metrics.get('total_loss', float('inf'))\n",
                    "    if acc > best_accuracy or tot_loss < best_loss:\n",
                    "        if acc > best_accuracy:\n",
                    "            best_accuracy = acc\n",
                    "        if tot_loss < best_loss:\n",
                    "            best_loss = tot_loss\n",
                    "        print(f'[*] New best metric! Peak Accuracy: {best_accuracy*100:.2f}%, Best Loss: {best_loss:.4f} (Saving Checkpoint...)')\n",
                    ...
                    "        }, checkpoint_path)\n",
                    "        print(f'[*] Checkpoint saved: {checkpoint_path} ({checkpoint_path.stat().st_size / (1024**2):.2f} MB)')\n"
                ]
            },
            {
                "cell_type": "markdown",
