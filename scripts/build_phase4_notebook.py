        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    print(f'Primary GPU: {gpu_name} ({gpu_mem:.2f} GB VRAM)')\n",
                    "    \n",
            ...
            },
            {
                ...
                "metadata": {},
                "source": [
                    "## 3. Model Instantiation & Parameter Verification (120,179,360 Parameters)"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "model = CognitiveReasoner120M(config).to(device=device, dtype=compute_dtype)\n",
                    "\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# Auto-reload cached module if already loaded in this Jupyter kernel\n",
                    "for mod_name in ['cir_arc.neural.training', 'cir_arc.neural.training.reasoning_dataset']:\n",
                    ...
                    "    )\n",
                    "except ImportError:\n",
                    "    print('Stale repository checkout detected in Kaggle. Updating to latest remote origin/master...')\n",
                    "    !git fetch origin master\n",
                    "    !git reset --hard origin/master\n",
                    "    for mod_name in ['cir_arc.neural.training', 'cir_arc.neural.training.reasoning_dataset']:\n",
                    "        if mod_name in sys.modules:\n",
                    ...
                    "            collate_reasoning_batch,\n",
                    "        )\n",
                    "        from scripts.generate_data import generate_single_rule_data, generate_composition_data\n",
                    "        def ensure_synthetic_data(output_dir='data/synthetic', n_per_rule=800, n_per_pair=400):\n",
                    "            p = Path(output_dir)\n",
                    "            generate_single_rule_data(n_per_rule, p)\n",
                    "            generate_composition_data(n_per_pair, p)\n",
                    "            return len(list((p / 'train').rglob('*.json'))), len(list((p / 'held_out').rglob('*.json')))\n",
                    "\n",
                    "train_data_dir = 'data/synthetic/train'\n",
                    "val_data_dir = 'data/synthetic/held_out'\n",
                    "\n",
                    "# Check if synthetic dataset exists. If missing (e.g. fresh git clone where data/ is git-ignored), generate automatically!\n",
                    ...
                    "    print('=' * 75)\n",
                    "    train_count, held_count = ensure_synthetic_data(\n",
                    "        output_dir='data/synthetic',\n",
                    "        n_per_rule=800,  # ~12,000 tasks total (9,600 train, 2,400 held_out)\n",
                    "        n_per_pair=400,\n",
                    "    )\n",
                    "    print(f'Procedural generation complete: {train_count:,} train and {held_count:,} held-out tasks.')\n",
                    "\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    \n",
                    "    intent, scores = model.plan(slot_embeddings=test_slots, candidate_actions=candidate_actions)\n",
                    "    \n",
                    "print('=' * 60)\n",
