...
def reconstruction_loss(
    ...
    ce_loss = F.cross_entropy(logits_perm, target_grid, reduction="none", ignore_index=ignore_index)
...
class ReconstructionLoss(nn.Module):
    ...
    def __init__(self, ignore_index: int = 10, eps: float = 1e-8):
        super().__init__()
    ...
    def forward(
        ...
        return reconstruction_loss(
            pred_logits, target_grid, mask=mask, ignore_index=self.ignore_index, eps=self.eps
        )
